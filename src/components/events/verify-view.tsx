...
export function VerifyView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!active) return;
      if (row?.valid) { setData({ full_name: row.full_name, event_name: row.event_name, issued_on: row.issued_on }); setState("valid"); }
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface p-8 text-center">
        ...
        {state === "valid" && data && (
          <>
            ...
            <h1 className="font-display text-xl font-bold text-heading">Verified certificate</h1>
