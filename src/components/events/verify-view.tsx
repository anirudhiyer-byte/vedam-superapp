...
export function VerifyView() {
  ...
  const [data, setData] = useState<{ full_name: string; event_name: string; issued_on: string; kind?: string; position?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!active) return;
      if (row?.valid) { setData({ full_name: row.full_name, event_name: row.event_name, issued_on: row.issued_on, kind: row.kind, position: row.position }); setState("valid"); }
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface p-8 text-center">
        ...
        {state === "valid" && data && (
          <>
            ...
            {data.kind === "winner" && <span className="mt-1 inline-block rounded-full bg-[linear-gradient(120deg,#B8860B,#F5C542)] px-3 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wide text-[#3a2a00]">{data.position === "1st" ? "🥇 1st place" : data.position === "2nd" ? "🥈 2nd place" : data.position === "3rd" ? "🥉 3rd place" : "🏆 Winner"}</span>}
