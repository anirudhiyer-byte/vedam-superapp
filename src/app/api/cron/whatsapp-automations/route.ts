...
type Recip = { phone: string; name: string; state: string | null; city: string | null };
...
async function audience(db: ReturnType<typeof admin>, a: Auto): Promise<Recip[]> {
  ...
  if (a.trigger_type === "scheduled") {
    let q = db.from("profiles").select("full_name, phone, state, city").not("phone", "is", null);
    if (st) q = q.eq("state", st);
    const { data } = await q;
    return (data ?? []).filter((p) => p.phone).map((p) => ({ phone: p.phone as string, name: p.full_name || "there", state: p.state, city: p.city }));
  }
  // event triggers
  let q = db.from("event_registrations").select("user_id, full_name, whatsapp, joined, profiles(phone, state, city, full_name)").eq("event_id", a.event_id);
  if (a.conditions?.attended === "yes") q = q.eq("joined", true);
  if (a.conditions?.attended === "no") q = q.eq("joined", false);
  const { data } = await q;
  const out: Recip[] = [];
  type Row = { full_name: string | null; whatsapp: string | null; profiles: { phone: string | null; state: string | null; city: string | null; full_name: string | null } | { phone: string | null; state: string | null; city: string | null; full_name: string | null }[] | null };
  for (const r of (data ?? []) as unknown as Row[]) {
    const prof = Array.isArray(r.profiles) ? r.profiles[0] : r.profiles;
    const phone = r.whatsapp || prof?.phone; if (!phone) continue;
    const state = prof?.state ?? null;
    if (st && state !== st) continue;
    out.push({ phone, name: r.full_name || prof?.full_name || "there", state, city: prof?.city ?? null });
  }
  return out;
}
