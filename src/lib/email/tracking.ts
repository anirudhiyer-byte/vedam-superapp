...
export function injectTracking(html: string, eventId: string, toEmail?: string): string {
  ...
  const unsub = toEmail ? `<div style="text-align:center;padding:14px 0;font:400 11px Arial,sans-serif;color:#999">Don't want these? <a href="${BASE}/api/email/unsubscribe?e=${encodeURIComponent(toEmail)}" style="color:#999;text-decoration:underline">Unsubscribe</a></div>` : "";
